...
def nouvelle_fenetre (variable, StadeEquipes): 
    ...
    while j< nbEquipe:
        while i+j < nbEquipe:
            ...
            w=j+i
            print ("equipe "+str(j)+"\n"+"equipe "+str(w)+"\n")
            textAffichage = (str(equipes[j-1][0])+" "+str(equipes[j-1][5]))
            Button(fenetre_fille, text=textAffichage).pack(pady=1, padx=0)  ### test bouton
            ###Label(fenetre_fille, text= textAffichage).pack(pady=1, padx=0)
            textAffichage = (str(equipes[(w-1)][0])+" "+str(equipes[(w-1)][5]))
            Button(fenetre_fille, text=textAffichage).pack(pady=1, padx=0)  ### test bouton
            ###Label(fenetre_fille, text= textAffichage).pack(pady=1, padx=0)
            Label(fenetre_fille, text= " ").pack(pady=1, padx=0)
            ### test bouton
