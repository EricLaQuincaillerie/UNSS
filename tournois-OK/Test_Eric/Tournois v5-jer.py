...
from fenetreMultiplesStade_1 import *
...
nbDept = len(listeDept)     # comptage du nb de departement ### correctif sur tout programme nbDept utilisé partout sans doublon 
nbCol = 5               # nombre de colonne pour affichage
print ("nombre de département au total en France : ",nbDept)  # affichage du nombre de departement

if __name__ == '__main__':
    fen = Tk()
    fen.title('liste des '+str(nbDept)+' départements')     # titre de la fenetre avec le nombre de département
...
def importDeptSelect():
    ...
    for row in fileDept:
        ...
        SelectDept.append([row[0], row[1], row[2],0,0,0])     ## nouvelle version modif jerome  pour integrer des colonnes vides pour resultats
...
#class melangeEquipe:
    #def tirageAuSort():
        #melange=[]
        #NbEquipe=len(SelectDept) #calcul le nombre d équipes selectionées
        #X = [i for i in range(0, NbEquipe+1)]  # la séquence qui sera battue
        #melange == random.shuffle(X)
        #print(melange)
...
class Stade:
    def __init__(self, ligne, colonne):
        ...
        while k < colonne:
            ...
            ##Button.grid(row=0+ligneNumero, column=colonnePosition, sticky=NSEW)#position de la colonne titre
            ##Button.config(relief=GROOVE)
            ##titre1 = Label(fen2, text=NumeroStade, width=30) # titre de la colonne incrementée de 1 à chaque fois
            ##titre1.grid(row=0+ligneNumero,column=colonnePosition, sticky=NSEW)   #position de la colonne titre
            ##titre1.config(relief=GROOVE, bd=2,fg="black",bg="yellow",font="Courier 12 ") #couleur et autre du titre 
            
            while j < ligne and s < NbEquipe:   # le test de s évite les erreurs de dépassement du nombre d équipes'
                NumEquipe=X[s]
                
                ##textAffichage = (str(Dept[NumEquipe][0])+" "+str(Dept[NumEquipe][2])+"\n " +str(Dept[NumEquipe][3:6]))
                textAffichage = (str(Dept[NumEquipe][0])+" "+str(Dept[NumEquipe][2]))
                ...
                #e.grid(row=j+1+ligneNumero, column=colonnePosition, sticky=NSEW) #decalage des lignes et colonnes
                #e.insert(END, SelectDept[s][0:6]) # affiche chaque dept
                #e.insert(END, Dept[NumEquipe][0]) # affiche chaque dept
                #e.insert(END, Dept[NumEquipe][2]) # et affiche la region
                #e.insert(END, Dept[NumEquipe][3]) #resultat premier match
                #e.insert(END, Dept[NumEquipe][4]) #resultat second match
                #e.insert(END, Dept[NumEquipe][5]) #resultat troisieme match
