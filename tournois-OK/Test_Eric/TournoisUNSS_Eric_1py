...
from fenetreMultiplesStade_4 import *
...
if __name__ == '__main__':
    fen = Tk()
    fen.title('UNSS')     # titre de la fenetre avec le nombre de département
    fen.geometry("1400x900")
    fen.config(bg="white", bd=5)

    tgl = Checkbar(fen)
    tgl.config(relief=GROOVE, bd=5)
    tgl.grid(row=6,  column=0, padx=50)
    
    lbDept = Listbox(fen)   # crée une listBox des départements présents
    sbDept = Scrollbar(fen, orient=VERTICAL)
    sbDept.config(command=lbDept.yview, bg="Orange", bd=5)
    lbDept.config(yscrollcommand=sbDept.set, bd=5)
    lbDept.grid(row=6, column=3, sticky=N+S)
    sbDept.grid(row=6, column=3, sticky=N+S+E)

    def setAttendance():  # selectionne tous les départements
        for x in range(0, len(listeDept)):
            listeDept[x][3].set(1)
        btnSetAll.config(text='Tout décocher', bg="Orange", bd=5)
        btnSetAll.config(command=unSetAttendance, bg="Orange", bd=5)
        updateAttendance()
	
    def unSetAttendance():  # déselectionne les départements
        for x in range(0, len(listeDept)):
            listeDept[x][3].set(0)
        btnSetAll.config(text='Tout cocher', bg="Orange", bd=5)
        btnSetAll.config(command=setAttendance, bg="Orange", bd=5)
        updateAttendance()
    ...
    label_dept = Label(fen, text='liste des '+str(nbDept)+' départements', font="Courier 12 bold").grid(row=3, column=0, rowspan=2)
    case_vide(4, 0)
    btnQuit = Button(fen, text='Suivant', command=fen.destroy, bg="Orange", bd=5)
    btnQuit.grid(row=22, column=0, padx=0, pady=0, sticky=S)
    btnSetAll = Button(fen, text='Tout cocher', command=setAttendance, bg="Orange", bd=5)
    btnSetAll.grid(row=21, column=0,  padx=0, pady=0, sticky=S)
